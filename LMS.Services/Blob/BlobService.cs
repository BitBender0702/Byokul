...
namespace LMS.Services.Blob
{
    public class BlobService : IBlobService
    {
        ...
        public async Task<string> CompressAndUploadFileAsync(IFormFile asset, string containerName, bool showProgress)
        {
        ...
        public async Task<string> UploadFileAsync(Stream asset, string containerName, bool showProgress)
        {
            string imageFullPath = null;
            if (asset == null)
            {
            ...
            try
            {
                if (CloudStorageAccount.TryParse(config.Value.StorageConnection, out CloudStorageAccount cloudStorageAccount))
                {
                    ...
                    CloudBlockBlob blockBlob = cloudBlobContainer.GetBlockBlobReference(filename);
                    await blockBlob.UploadFromStreamAsync(asset);
                    imageFullPath = blockBlob.Uri.ToString();
                }
            }
            catch (Exception ex)
